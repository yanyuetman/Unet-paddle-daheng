...
def rotate_image(img, label):
    ...
    image_rotate = cv2.warpAffine(img, M, (w, h))
    label_rotate = cv2.warpAffine(label, M, (w, h))
    return image_rotate, label_rotate


def random_crop(img, label, size, scale=None, ratio=None):
    """ random_crop """
    ...
    img = img[i:i + w, j:j + h, :]
    label = label[i:i + w, j:j + h, :]

    img = cv2.resize(img, (size, size), interpolation=cv2.INTER_LANCZOS4)
    label = cv2.resize(label, (size, size), interpolation=cv2.INTER_LANCZOS4)
    return img, label
...
def _reader_creator(file_list,
    ...
    def reader():
        with open(file_list) as flist:
            ...
            full_lines = [line.strip('\n') for line in flist if line.find('image') != -1]
            if shuffle:
                if pass_id_as_seed:
            ...
            for line in lines:
                if mode == 'train' or mode == 'val':
                    ...
                    img_path = line
                    label_path = line.replace("image", "mask")
